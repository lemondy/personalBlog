      ...
      <article className="prose dark:prose-invert">
        ...
        <ul>
          ...
          <li>
            ...
            <li>金币激励策略通常能显著的拉新新用户、促进用户活跃、延长用户留存/活跃时间；但是红包/金币发出去的是真实成本，需要保持这个事项的ROI大于1才可以保证业务形态的可持续发展；</li>
            <li>通过奖励加衰减因子、不健康用户打压、金币敏感度模型、广告ECPM模型等手段在一个双月内实现成本降低50%，用户留存下降1%以内</li>
          </li>
        </ul>
        ...
        <ul>
          ...
          <li>计算用户对书籍的阅读、购买、收藏、投票等行为的TF-IDF值，作为行为对应的权重，然后乘以行为累积值，接着对分数进行线性累加，之后对分数进行缩放，计算出来用户对书籍的评分表，最后使用牛顿冷却公示对这个分数按照时间进行衰减；</li>
          <li>基于构建的人书分数，进一步训练书籍B2V向量，推荐服务使用书籍embedding向量进行相似书籍召回；</li>
          </li>
          <li>
